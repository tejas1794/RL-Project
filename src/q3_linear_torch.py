...
class Linear(DQN):
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        mse_loss_input = gamma * (1 - done_mask) * (torch.reshape(torch.max(target_q_values, dim=1, keepdim=True).values, (-1,))) + rewards
        q_a = torch.sum(torch.nn.functional.one_hot(actions, self.env.action_space.n) * q_values, dim=1)
        loss = torch.nn.functional.mse_loss(mse_loss_input, q_a)
        ##############################################################
        ######################## END YOUR CODE #######################
        return loss
