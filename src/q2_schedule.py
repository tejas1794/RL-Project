...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        eps_interp = self.eps_begin - (self.eps_begin - self.eps_end) * np.float32(t) / self.nsteps
        self.epsilon = self.eps_end if t > self.nsteps else eps_interp
        ##############################################################
        ######################## END YOUR CODE ############## ########
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        return self.env.action_space.sample() if np.random.rand() < self.epsilon else best_action
        ##############################################################
        ######################## END YOUR CODE #######################
